<template>
  <div>
    <fieldset  class="box">
      <legend class="is-size-5" style="background-color:white; margin-bottom: -35px"><b>Production</b></legend>
      <!-- <select v-model="ChosenTeamMember">
        <option disabled value="">Select a Team Member</option>
        <option v-for="teamMember in FrstShftPrdctnTmMmbrs" v-bind:value="teamMember.teamMemberID" v-bind:key="teamMember.teamMemberID" v-bind:class="{ 'noScore': (teamMember.scoreStatusID == noScore), 'scoreSaved': (teamMember.scoreStatusID == scoreSaved), 'scoreSubmitted': (teamMember.scoreStatusID == scoreSubmitted), 'teamMemberAbsent': (teamMember.scoreStatusID == teamMemberAbsent) }">{{ teamMember.firstName }} {{ teamMember.lastName }} {{ teamMember.scoreStatusID }}</option>
      </select> -->
      <!-- <span v-for="teamMember in FrstShftPrdctnTmMmbrs" v-bind:key="teamMember.fullName">
        <button v-bind:id="teamMember.teamMemberID" v-bind:value="teamMember" v-bind:key="teamMember.teamMemberID" v-on:click="getMasterScoreId()" class="button" v-bind:class="{ 'noScore': (teamMember.scoreStatusID == noScore), 'scoreSaved': (teamMember.scoreStatusID == scoreSaved), 'scoreSubmitted': (teamMember.scoreStatusID == scoreSubmitted), 'teamMemberAbsent': (teamMember.scoreStatusID == teamMemberAbsent) }">{{ teamMember.firstName }} {{ teamMember.lastName }}</button> -->        
      <!-- </span>       -->
      <ul class="radio-to-button">
        <li v-for="teamMember in FrstShftPrdctnTmMmbrs" v-bind:key="teamMember.fullName">
          ...
          <label v-bind:for="teamMember.teamMemberID" v-bind:class="{ 'noScore': (teamMember.scoreStatusID == noScore), 'scoreSaved': (teamMember.scoreStatusID == scoreSaved), 'scoreSubmitted': (teamMember.scoreStatusID == scoreSubmitted), 'teamMemberAbsent': (teamMember.scoreStatusID == teamMemberAbsent) }">{{ teamMember.firstName }} {{ teamMember.lastName }}</label>
        </li>
      </ul>
    </fieldset>
  </div>
...
export default {
  ...
  computed: {    
    ...
    FrstShftPrdctnTmMmbrs: {
      get () {
        // console.log /g(this.$store.state.FrstShftPrdctnTmMmbrs);
        return this.$store.state.FrstShftPrdctnTmMmbrs;
      }      
    }    
  },
  methods: {
    ...
    getMasterScoreId () {
      ...
      // DO THIS IF A SCORE DOES NOT EXIST FOR CHOSEN TEAM MEMBER
        ...
        let scoreTeamMemberID = this.ChosenTeamMember.teamMemberID,
            shiftTeamMemberID = this.ChosenTeamMember.teamMemberShiftID,
            positionName = this.ChosenTeamMember.teamMemberPosition,
            fullName = this.ChosenTeamMember.fullName,            
            scoreStatusID = 1,
            scoreDate = this.DateToScore,
            scoreCreatedDateTime = moment(Date()).format(),
            positionTeamMemberID = this.ChosenTeamMember.teamMemberPosition,    departmentTeamMemberID = this.ChosenTeamMember.teamMemberDepartmentCode,
